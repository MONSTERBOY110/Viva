...
Every other interview agent is something you watch. Viva is something you can drive.

While an interview is running, the Interviewer Mind panel is not just a readout. An observer can push the examiner mid-flight: **press harder**, **ease off**, **move on**, **wrap up**, or send it straight to a specific curriculum day. The examiner obeys in character on its very next question, and the panel records who asked for what.

The interesting part is what happens when a steer conflicts with the rules. **It loses.** Steering is a preference, not an override: the deterministic guardrails still run afterwards, so an observer can shape the interview but cannot take it below the required question count, the curriculum coverage floor, or the difficulty ceiling. Steer `wrap` at question two and the interview politely carries on, with the rationale recording the whole chain of custody:

> The candidate defined embeddings but failed to address why SQL is insufficient. **[steered by observer: wrap]** **[policy: difficulty increase blocked after a weak answer]**

Steering lives on its own route, `POST /api/session/[id]/steer`, precisely so the graded contract endpoint never changes shape.

## Replay the reasoning

The live panel is the best thing about watching an interview, and it is gone by the time anyone reads the report. So the report rebuilds it. Scrub through the interview question by question and watch coverage fill, confidence move, and each guardrail fire, exactly as it stood at that moment.

Nothing there is regenerated by a model. Coverage and confidence are replayed from the recorded evaluations using the same rule the live engine used, so the replay is evidence rather than a re-enactment.

## How the answer arrived

Every answer records how it was given: typed over 74 seconds at 96 characters per minute, spoken, or pasted in 2. The report shows it inline in the transcript.

This is deliberately framed as observation, not accusation. A pasted answer proves nothing on its own, it is simply a fact a human interviewer in the room would have noticed, and it is the sort of signal a real interview product cannot ignore.
